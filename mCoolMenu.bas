...
Public m_SideBitmapWidth As Long

Private pmds As CMyItemDatas 'the collection of pmd
Public WndCol As Collection 'the collection of WndCoolMenu

Public Sub ConvertMenu(hwnd As Long, hMenu As Long, nIndex As Long, bSysMenu As Boolean, bShowButtons As Boolean, Optional Permanent As Boolean = False)
...
Public Sub OnInitMenuPopup(hwnd As Long, hMenu As Long, nIndex As Long, bSysMenu As Boolean)
'Based on Paul DiLascia's
...
Public Function OnMenuChar(nChar As Long, nFlags As Long, hMenu As Long) As Long
'Based on Paul DiLascia's
...
Public Function OnDrawItem(hwnd As Long, ByRef dsPtr As Long, Optional bOverMain As Boolean = False) As Boolean
'Based on Paul DiLascia's
'Draw menu items
...
Public Function OnMeasureItem(hwnd As Long, ByRef miPtr As Long) As Boolean
'Based on Paul DiLascia's
'Mesure menu items
...
'    m_SideBitmapWidth = 32
  ...
  Else
    ...
    lpms.itemWidth = itemWidth& + mCoolMenu.m_SideBitmapWidth
  End If
...
Public Function Install(wndHandle As Long, Optional Subclass As Boolean = True, Optional HelpObj As HelpCallBack, Optional ilHandle As Object) As Boolean
'Install CoolMenu on the specified window handle
  ...
  If wndHandle <> 0 Then
    ...
    If Subclass Then
        ...
        Call SetWindowLong(wndHandle&, GWL_WNDPROC, AddressOf mWindowProc)
    End If
...
Public Function Uninstall(wndHandle As Long, Optional Subclass As Boolean = True) As Boolean
'Unintall sublassing on a window by handle

  If (wndHandle <> 0) And (Not (WndCol Is Nothing)) Then
    
    If Subclass And WndCol(CStr(wndHandle&)).PrevProc <> 0 Then
...
Public Sub OnMenuSelect(hwnd As Long, nItemID As Integer, nFlags As Integer, hSysMenu As Long)
...
Public Function OnDrawMainMenu(hwnd As Long, lParam As Long, MousePosition As Long) As Long
...
Public Function mWindowProc(ByVal hwnd As Long, ByVal Msg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
  On Error GoTo ErrorHandle
...
'        Debug.Print "WM_INITMENUPOPUP " & LoWord(lParam&); HiWord(lParam&); wParam&
    ...
    Case WM_MENUCHAR:
'        Debug.Print "WM_MENUCHAR " & LoWord(lParam&); HiWord(lParam&); LoWord(wParam&); HiWord(wParam&)
...
Continue:
  mWindowProc& = CallWindowProc(WndCol(CStr(hwnd&)).PrevProc, hwnd&, Msg&, wParam&, lParam&)
  Exit Function
ErrorHandle:
